...
非离散型: 连续型, 一个或多个区间
  ...
  <img src="https://github.com/lhcezx/Notes/raw/main/Images/二项分布.jpeg" alt="图片无法显示" height=200px>
   设事件A发生的概率为 $P(A) = P$, $n$ 次实验发生了 $k$ 次, 则 $P(X=k)=C_{n}^{k}p^{k}(1-p)^{1-k}$, 其中 $k=0,1,2....n$, 我们将其记做 $X\sim B(n,p)$
  ...
  4) 标准正态分布
  定义: $\mu = 0, \sigma = 1$. 其密度函数为 $phi(x)_ 0=\frac{1}{\sqrt{2\pi}} e^{-\frac{x^2}{2}}, -\infty<x<+\infty$
  
  分布函数:
  $\Phi(x)_ 0 =\frac{1}{\sqrt{2\pi}}\int_{-\infty}^{x} e^{-\frac{t^2}{2}}dt$
  
  性质:
  - 以 $y$ 轴为对称轴
  - 概率密度函数是偶函数, $\phi(x)_ 0 = \phi(-x)_ 0$ 
  - $\Phi(x)_ 0 = 1-\Phi(-x)_ 0$ 
