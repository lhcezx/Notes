...
非离散型: 连续型, 一个或多个区间
  ...
  ## 3.1.1 二维离散型随机变量
  联合概率分布: 通常我们会构造一个分布表, 其中 $P(X=x_i, Y=y_i) = P_{ij}$, $F(x,y)=P(X\leq x, Y\leq y) = \sum_{X_i\leq x} \sum_{Y_j\leq y} P_{ij}$. 其中 $f($
  ...
  - 边缘分布不能确定联合分布, 只有在 $X, Y$ 独立的情况下可以确定其联合分布
   ...
   ## 3.1.3 连续型条件分布
   条件分布定义: $(X,Y)$ 是个二维随机变量, $f(x,y)$ 为其二维概率密度函数, 边缘密度函数 $f_X(x), f_Y(y)$ 是已知的情况下, 若 $f_Y(y\geq 0)$ (即 $y$ 已经发生), 则条件分布函数为 $F(x|y) = \int_{-\infty}^{x} \frac{f(u,y)}{f_Y(y)} du$, $f(x|y) = \frac{f(x,y)}{f_Y(y)}$ 
   ...
   概率定义: $P(X\leq x|Y = y)= \frac{P(X\leq x, Y=y)}{P(Y=y)}$
