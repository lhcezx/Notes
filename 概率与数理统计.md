...
非离散型: 连续型, 一个或多个区间
  ...
  - 注意 $F_X(x) = P(X\leq x), F_Y(x)=P(Y\leq x)$. 需要注意的是 $F_Y(x)$ 中的变量仍然为 $x$. $Y, X$ 代表随机变量, $x$ 则代表随机变量的取值. 由于随机变量 $Y$ 由随机变量 $X$ 组成, 因此其取值也是在 $X$ 的维度空间 $x$. 因此 $Y$ 在这里依然为一维随机变量而非 $(x, y)$ 构成的二维随机变量.
