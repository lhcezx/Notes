...
非离散型: 连续型, 一个或多个区间
  ...
  3) 二项分布: 设事件A发生的概率为 $P(A) = P$, $n$ 次实验发生了 $k$ 次, 则 $P(X=k)=C_{n}^{k}p^{k}(1-p)^{1-k}$, 其中 $k=0,1,2....n$, 我们将其记做 $X\sim B(n,p)$
  ...
  5) 超几何分布: 假设 $N$ 个元素: $N_1$ 个属于第一类, $N_2$ 个属于第二类. 从中取 $n$ 个元素, $X$ 表示取的 $n$ 个中属于第一类的个数. $P(X=k)=\frac{C_{N_1}^{k}C_{N_2}^{n-k}}{C_{N}^{n}}$. $ k=0,1,2... min(n, N_1) $. 超几何分布可以描述不放回抽样的实验.\
