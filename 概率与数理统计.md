...
非离散型: 连续型, 一个或多个区间
  ...
  边缘分布: $F_X(x_i) = P(X\leq x, Y<+\infty) = \sum_{X_i\leq x} \sum_{j} P_{ij}$
  ...
  - 边缘分布不能确定联合分布, 只有在 $X, Y$ 独立的情况下可以确定其联合分布
   ...
   联合密度函数: $F(x,y) = P(X\leq x, Y\leq y) = \int_{-\infty}^{x} \int_{-\infty}^{y} f(s,t)dsdt$
   ...
   边缘密度函数(求导): $f_X(x) = \int_{\infty}^{+\infty}f(x,t)dt = \int_{\infty}^{+\infty}f(x,y)dy, f_Y(y) = \int_{\infty}^{+\infty}f(s,y)ds = \int_{\infty}^{+\infty}f(x,y)dx$
