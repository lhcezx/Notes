...
非离散型: 连续型, 一个或多个区间
  ...
  - 边缘分布不能确定联合分布, 只有在 $X, Y$ 独立的情况下可以确定其联合分布
   ...
   ## 3.1.3 连续型条件分布
   定义: $(X,Y)$ 是个二维随机变量, $f(x,y)$ 为其二维概率密度函数, 边缘密度函数 $f_X(x), f_Y(y)$ 是已知的情况下, 若 $f_Y(y\geq 0)$ (即 $y$ 已经发生), 则条件分布函数为 $F(x|y) = \int_{-\infty}^{x} \frac{f(u,y)}{f_Y(y)} du$, $f(x|y) = \frac{f(x,y)}{f_Y(y)}$ 
